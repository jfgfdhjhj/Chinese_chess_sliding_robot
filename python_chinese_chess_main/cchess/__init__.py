...
class Piece:
    ...
    def _repr_svg_(self) -> str:
        import python_chinese_chess_main.cchess.svg
        return python_chinese_chess_main.cchess.svg.piece(self, size=45)
...
class BaseBoard:
    ...
    def _repr_svg_(self):
        import python_chinese_chess_main.cchess.svg
        return python_chinese_chess_main.cchess.svg.board(board=self, size=600, axes_type=self._axes_type, style=self._svg_css)
    ...
    def unicode(self, *, invert_color: bool = False, axes: bool = True, axes_type: int = 0) -> str:
        ...
        for row_index in range(9, -1, -1):
            ...
            for col_index in range(9):
                ...
                if piece:
                    char = piece.unicode_symbol(invert_color=invert_color)
                    if char in ["俥", "傌", "相", "仕", "帥", "炮", "兵"]:
                        char = RED_COLOR + char + RESET_COLOR
                    # else:
                    #     char = BLACK_COLOR  + char + RESET_COLOR
                    builder.append(char)
                else:
                    builder.append("．")
...
class Board(BaseBoard):
    ...
    def _repr_svg_(self) -> str:
        import python_chinese_chess_main.cchess.svg
        return python_chinese_chess_main.cchess.svg.board(board=self,
                                                            size=450,
