...
import os
from chess_utils.logger import logger as mylogger
import subprocess
import threading
import time
import logging
...
def my_main():
    ...
    engine = UCCIEngine(engine_address, callback=None)
    engine.start()
    time.sleep(0.1)
    logger.debug(engine.isready())
    engine.position("rnbakabnr/9/1c5c1/p1p1p1p1p/9/9/P1P1P1P1P/1C5C1/9/RNBA1ABNR r ")
    engine.go(depth=20)
    engine.join()
    engine.close()
...
def main():
    ...
    # def get_user_move():
    #     # 获取用户输入的移动
    #     user_input = input("请输入您的移动（例如：a9a7）：")
    #     # 将用户输入的移动转换为引擎能够理解的格式（例如：a9a7 -> (0, 0)）
    #     move = ((int(user_input[1]) - 1, ord(user_input[0]) - ord('a')),
    #             (int(user_input[3]) - 1, ord(user_input[2]) - ord('a')))
    #     return move
    ...
    def callback(move_type, data):
        if move_type in (Chess.MOVE,):
            ...
            time.sleep(1)
            engine.position()
            # engine.position("rnbakabnr/9/1c5c1/9/9/9/9/1C5C1/9/RNBA1ABNR r ")
            ...
            while engine.undo():
                ...
                time.sleep(0.01)
    engine = UCCIEngine(engine_address, callback=callback)
    engine.start()
    # engine.clear()
    logger.debug(engine.isready())
    engine.position("rnbakabnr/9/1c5c1/9/9/9/9/1C5C1/9/RNBA1ABNR r ")
